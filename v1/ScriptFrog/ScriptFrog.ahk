...
#Include %A_ScriptDir%\double click blocker.ahk
...
ValorantOverride:
	ActiveTitle := "Valorant"
ValorantHotkeys:
	If InStr(ActiveTitle, "VALORANT") {
		...
		If !(ValorantEnabled) {
			...
			SaveHotkeys("default")
			RestoreHotkeys("Valorant")
			RemoveDuplicateHotkeys()
			ValorantEnabled=1
			Gosub SB_Profile
			DebugPrepend("Enabled Valorant")
		}
		Return
	} else {
		If (!ValorantEnabled or ActiveTitle="Search" or !ActiveTitle or ActiveTitle=GuiTitle) {
			Return
		} else {
			If InStr(ActiveTitle, "VALORANT") {
				Return
...
DisableValorant:
	SaveHotkeys("Valorant")
	RestoreHotkeys("default")
	RemoveDuplicateHotkeys()
	ValorantEnabled=0
	Gosub SB_Profile
	DebugPrepend("Disabled Valorant")
Return
